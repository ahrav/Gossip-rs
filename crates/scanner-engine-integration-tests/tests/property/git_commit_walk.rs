...
//!
//! These tests generate random DAGs and verify two key invariants:
//!
//! 1. **Range correctness**: the set of commits selected by `CommitPlanIter`
//!    for a given tip/watermark pair matches a naive reachability computation.
//! 2. **Topological ordering**: parents always appear before children, and
//!    each commit's first reachable descendant in the order is itself.
//!
//! DAGs are generated by only allowing parents from lower-indexed positions,
...
impl CommitGraph for TestCommitGraph {
    ...
    fn root_tree_oid(&self, pos: Position) -> Result<OidBytes, CommitPlanError> {
        Ok(TestCommitGraph::oid_for_pos(pos.0))
    }
...
/// Generates a random DAG where each node's parents are drawn from lower indices, guaranteeing acyclicity.
...
/// Extends `dag_strategy` with random tip/watermark pairs for range-walk tests.
...
/// Extends `dag_strategy` with random "secret introduction" commits for topological ordering tests.
...
proptest! {
    ...
    fn topo_order_preserves_first_introduction(
        ...
        let order = scanner_git::topo_order_positions(
            &graph,
            &positions,
