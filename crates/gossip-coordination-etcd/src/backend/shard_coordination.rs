...
//!
//! # Sync and async structure
//!
//! The synchronous backend implementation is generated by
//! [`impl_sync_coordination_backend!`]. [`CoordinationBackend`] methods take
//! `&mut self`, and each CAS retry path also borrows `self`, so a blanket impl
//! would introduce borrow patterns the trait cannot express cleanly. The macro
//! keeps those monomorphized implementations close to the shared helper logic.
//!
//! The asynchronous backend stays handwritten because each retry attempt owns
//! futures, temporary buffers, and lease cleanup work differently from the
//! synchronous path. Both variants still share the same storage protocol:
//! mutate the shard record, verify the owner binding, update index/counter keys
//! in the same transaction, then perform lease revocation or cache refresh as
//! best-effort follow-up work after the CAS commits.
...
///   after idempotency and lease/owner validation succeed, so the closure can
///   focus on operation-specific field changes.
///
...
///
/// If the retry budget is exhausted, the helper reloads the shard to
/// distinguish replay and stale-fence cases from unexpected contention before
/// synthesizing the terminal backend error.
...
impl AsyncEtcdCoordinator {
    ...
    /// function's doc comment for the shared protocol semantics. This variant
    /// follows the same post-exhaustion re-load pattern so replay and
    /// stale-fence outcomes win over a generic retry-budget failure.
    #[allow(clippy::too_many_arguments)]
...
impl AsyncCoordinationBackend for AsyncEtcdCoordinator {
    /// Acquire ownership and restore persisted shard state via async CAS.
    ///
    /// Mirrors [`CoordinationBackend::acquire_and_restore_into`] while routing
    /// etcd requests through the async helpers so the worker can yield while
    /// waiting. Grants a new lease, bumps the fence epoch, and atomically
    /// writes both the updated shard record and `/owner` key before building
    /// the restored view for the caller.
    ///
    /// # Errors
    ///
    /// `AcquireError` variants cover missing shards, terminal status, stale
    /// vs. already-leased shards, tenant mismatches, and backend failures
    /// (RPC or codec corruption).
    ...
    /// Extend a shard lease deadline without changing ownership via async CAS.
    ///
    /// Validates the presented lease binding, increments the logical deadline,
    /// and writes the updated deadline just like the synchronous backend, then
    /// best-effort keeps the etcd lease alive.
    ///
    /// # Errors
    ///
    /// `RenewError::ShardNotFound`, `RenewError::StaleFence`, or
    /// `RenewError::BackendError` explain why the renewal failed.
    async fn renew(
        &mut self,
        now: LogicalTime,
        tenant: TenantId,
    ...
    /// Persist a cursor update via async CAS while keeping the operation
    /// idempotent.
    ///
    /// Validates the lease, ensures cursor monotonicity/bounds, and appends an
    /// op-log entry before CAS-writing the refreshed shard record, mirroring
    /// the synchronous `checkpoint` semantics.
    ///
    /// # Errors
    ///
    /// `CheckpointError::ShardNotFound`, `CheckpointError::StaleFence`, and
    /// `CheckpointError::BackendError` describe the failure cases.
    async fn checkpoint(
        &mut self,
        now: LogicalTime,
        tenant: TenantId,
    ...
    /// Terminal transition to `Done` status with async CAS semantics.
    ///
    /// Validates the lease and cursor, sets the status to `ShardStatus::Done`,
    /// clears the logical lease, and routes through the shared async terminal
    /// transition helper so the same CAS guard and idempotency rules apply. A
    /// replay returns `IdempotentOutcome::Replayed(())`.
    ///
    /// # Errors
    ///
    /// `CompleteError` variants track shard-not-found, lease validation
    /// failures, or backend issues.
    async fn complete(
        &mut self,
        now: LogicalTime,
        tenant: TenantId,
    ...
    /// Park a shard with a reason via async CAS semantics.
    ///
    /// Routes through the shared async terminal transition helper with
    /// `ShardStatus::Parked`, records the provided [`ParkReason`], clears the
    /// lease, and preserves the idempotent op-log guard.
    ///
    /// # Errors
    ///
    /// `ParkError` variants cover lease validation failures and backend faults.
    async fn park_shard(
        &mut self,
        now: LogicalTime,
        tenant: TenantId,
    ...
    /// Replace an owned shard with N child shards using async CAS semantics.
    ///
    /// Mirrors the synchronous `split_replace`: the parent transitions to
    /// `ShardStatus::Split`, its owner key and active-index entry are deleted,
    /// and each child is created as an unowned `Active` shard with deterministic
    /// IDs. Derived-ID replay and collision checks behave the same while the
    /// etcd operations are issued through the async transaction helpers.
    ///
    /// # Errors
    ///
    /// `SplitReplaceError` variants surface missing shards, tenant mismatch,
    /// terminal parents, validation failures, limit checks, derived-ID
    /// collisions, or backend issues.
    ...
    /// Shrink a shard and spawn a residual using async CAS semantics.
    ///
    /// Keeps the parent `Active`, retains its owner binding, and atomically
    /// narrows the parent's key range while creating an unowned residual shard
    /// that covers the removed range. Derived residual IDs and tenant/global
    /// counters share the synchronous precondition and replay logic.
    ///
    /// # Errors
    ///
    /// `SplitResidualError` variants cover missing shards, tenant mismatch,
    /// terminal parents, validation failures, stale leases, derived-ID
    /// collisions, and backend faults.
