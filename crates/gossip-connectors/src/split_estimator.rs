...
mod tests {
    ...
    #[test]
    fn all_zero_sizes_fall_back_to_count_midpoint() {
        let mut estimator = StreamingSplitEstimator::default();
        for idx in 0..9 {
            let key = key_for_index(idx);
            estimator.observe(&key, 0);
        }
        let key = estimator
            .estimate_split_key()
            .expect("midpoint split should exist");
        assert_eq!(index_from_key(&key), 4);
    }
    ...
    #[test]
    fn key_sample_memory_remains_bounded_for_large_streams() {
        let mut estimator = StreamingSplitEstimator::new(64);
        for idx in 0..1_000_000usize {
            let key = key_for_index(idx);
            estimator.observe(&key, 0);
        }
        assert!(estimator.rank_sample_len() <= estimator.sample_cap());
        assert!(estimator.byte_sample_len() <= estimator.sample_cap());
        assert!(estimator.pending_len() <= estimator.pending_cap());
    }
}
