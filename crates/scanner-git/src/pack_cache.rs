...
mod proptests {
    ...
    fn arb_standard_ops() -> impl Strategy<Value = Vec<CacheOp>> {
        prop::collection::vec(
            prop_oneof![
                (0usize..STANDARD_ENTRY_COUNT).prop_map(CacheOp::Get),
                (0usize..STANDARD_ENTRY_COUNT).prop_map(CacheOp::Insert),
            ],
            48..96usize,
        )
        .prop_map(|mut ops| {
            // Seed every catalog entry so early Get ops can hit. Without
            // this, random Gets on un-inserted indices always miss, starving
            // DependencyClock hint coverage.
            let mut seeded: Vec<CacheOp> = (0..STANDARD_ENTRY_COUNT).map(CacheOp::Insert).collect();
    ...
    fn arb_same_set_ops() -> impl Strategy<Value = Vec<CacheOp>> {
        prop::collection::vec(
            prop_oneof![
                (0usize..SAME_SET_ENTRY_COUNT).prop_map(CacheOp::Get),
                (0usize..SAME_SET_ENTRY_COUNT).prop_map(CacheOp::Insert),
            ],
            24..72usize,
        )
        .prop_map(|mut ops| {
    ...
    fn arb_same_set_workload(
        ...
        // exercised, not just set 0.
        (0usize..4, arb_same_set_entry_specs(), arb_same_set_ops())
    }
    ...
    fn arb_same_set_large_ops() -> impl Strategy<Value = Vec<CacheOp>> {
        prop::collection::vec(
            prop_oneof![
                (0usize..SAME_SET_LARGE_ENTRY_COUNT).prop_map(CacheOp::Get),
                (0usize..SAME_SET_LARGE_ENTRY_COUNT).prop_map(CacheOp::Insert),
            ],
            24..72usize,
        )
        .prop_map(|mut ops| {
    ...
    fn run_oracle_workload(
        ...
        // Post-workload integrity sweep: query every catalog entry to catch
        // cross-slot corruption that the random op sequence may have missed.
    ...
    proptest! {
        ...
        #[test]
        fn ensure_capacity_grow_clears_entries(
            (catalog_specs, ops) in arb_standard_workload(),
        ) {
            let mut cache = new_standard_oracle_cache(EvictionPolicy::Clock);
            let catalog = build_catalog(&catalog_specs);
        ...
        /// ensure_capacity with an equal-or-smaller value resets without grow.
        #[test]
        fn ensure_capacity_noop_resets_entries(
            (catalog_specs, ops) in arb_standard_workload(),
        ) {
            let mut cache = new_standard_oracle_cache(EvictionPolicy::Clock);
            let catalog = build_catalog(&catalog_specs);
        ...
        /// Re-inserting the same offset with different data overwrites the entry.
        #[test]
        fn duplicate_insert_overwrites(
            ...
            let mut cache = new_standard_oracle_cache(EvictionPolicy::Clock);
            let payload_a = materialize_payload(offset, kind_a, PayloadSpec { len, seed: seed_a });
            let payload_b = materialize_payload(offset, kind_b, PayloadSpec { len, seed: seed_b });
