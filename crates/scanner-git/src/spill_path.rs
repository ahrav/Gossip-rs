...
/// Using a typed `kind` keeps callers from smuggling path separators into
/// what should remain a single filename component.
pub(crate) fn make_unique_spill_path(dir: &Path, kind: SpillPathKind) -> PathBuf {
...
#[cfg(test)]
mod tests {
    use super::{make_unique_spill_path, SpillPathKind};

    #[test]
    fn spill_path_kind_controls_filename_prefix() {
        let dir = std::path::Path::new("spill-root");
        let blob = make_unique_spill_path(dir, SpillPathKind::Blob);
        let tree = make_unique_spill_path(dir, SpillPathKind::Tree);

        assert_eq!(blob.parent(), Some(dir));
        assert_eq!(tree.parent(), Some(dir));
        assert!(blob
            .file_name()
