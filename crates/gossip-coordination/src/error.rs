...
use gossip_contracts::coordination::shard_spec::{
    CursorSemantics, ShardSpecRef, SplitValidationError,
};
...
impl<const N: usize> FixedBuf<N> {
    ...
    #[inline]
    pub fn write(&mut self, bytes: &[u8], oversized_msg: &'static str) {
    ...
    /// Panics if `bytes.len() > N` with `oversized_msg`.
    #[inline]
    pub fn from_slice(bytes: &[u8], oversized_msg: &'static str) -> Self {
...
pub struct AcquireScratch {
    ...
    spec_metadata: FixedBuf<{ gossip_contracts::coordination::shard_spec::MAX_METADATA_SIZE }>,
...
impl AcquireScratch {
    ...
    pub fn write_spec(&mut self, start: &[u8], end: &[u8], metadata: &[u8]) {
        self.spec_start
    ...
    pub fn write_cursor(&mut self, last_key: Option<&[u8]>, token: Option<&[u8]>) {
        match last_key {
            Some(last_key) => {
    ...
    pub fn write_spawned_iter<I>(&mut self, spawned: I)
    where
