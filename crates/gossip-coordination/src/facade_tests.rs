...
fn acquire_lease(
    ...
    key: ShardKey,
    worker: WorkerId,
) -> Result<crate::Lease, crate::AcquireError> {
    let mut scratch = crate::AcquireScratch::new();
    coord
...
fn claim_cooldown_spans_runs() {
    ...
    let spec_a =
        gossip_contracts::coordination::shard_spec::ShardSpec::with_range(vec![0], vec![1]);
    ...
    let spec_b =
        gossip_contracts::coordination::shard_spec::ShardSpec::with_range(vec![10], vec![11]);
