...
use crate::facade::{ClaimError, ShardClaiming, default_claim_next_available};
...
pub const DEFAULT_MAX_AUTO_SLAB_CAPACITY: usize = 64 * 1024 * 1024;
...
fn split_residual_validate_cursor_bounds(
    ...
    {
        return Err(SplitResidualError::SplitInvalid(
            gossip_contracts::coordination::shard_spec::SplitValidationError::ParentCursorOutOfBounds {
                cursor: k.len(),
...
impl InMemoryCoordinator {
    ...
    pub fn count_available_for_run(
        &self,
        now: LogicalTime,
        tenant: TenantId,
        run: RunId,
...
impl RunManagement for InMemoryCoordinator {
    ...
    fn unpark_shard(
        ...
        if check_op_idempotency(record, op_id, payload_hash)
            .map_err(|e| match e {
                crate::error::CoordError::OpIdConflict {
                    op_id,
                    expected_hash,
                    actual_hash,
                } => UnparkError::OpIdConflict(crate::run::RunOpIdConflict {
                    op_id,
                    expected_hash,
                    actual_hash,
                }),
                crate::error::CoordError::ShardNotFound { .. }
                | crate::error::CoordError::TenantMismatch { .. }
                | crate::error::CoordError::StaleFence { .. }
                | crate::error::CoordError::LeaseExpired { .. }
                | crate::error::CoordError::ShardTerminal { .. }
                | crate::error::CoordError::CursorRegression { .. }
                | crate::error::CoordError::CursorOutOfBounds(_)
                | crate::error::CoordError::CursorKeyTooLarge { .. }
                | crate::error::CoordError::CursorTokenTooLarge { .. }
                | crate::error::CoordError::SplitInvalid(_)
                | crate::error::CoordError::CheckpointMissingKey => {
...
pub struct InMemorySpawnedIter<'a> {
    inner: gossip_contracts::coordination::pooled::PooledSpawnedIter<'a>,
}
