    ...
    let shard_entries: Vec<_> = (0..shard_count)
        .map(|i| {
            ...
            (
                ...
                gossip_contracts::coordination::shard_spec::ShardSpec::with_range(start, end),
...
fn split_residual_keeps_session() {
    ...
    let parent_new_spec =
        gossip_contracts::coordination::shard_spec::ShardSpec::with_range(vec![0x00], vec![0x20]);
    let residual_spec =
        gossip_contracts::coordination::shard_spec::ShardSpec::with_range(vec![0x20], vec![0x40]);
    let plan =
...
fn successive_split_residual_accumulates_spawned() {
    ...
    let shard_spec =
        gossip_contracts::coordination::shard_spec::ShardSpec::with_range(vec![0x00], vec![0x60]);
    ...
    let parent_new_1 =
        gossip_contracts::coordination::shard_spec::ShardSpec::with_range(vec![0x00], vec![0x40]);
    let residual_1 =
        gossip_contracts::coordination::shard_spec::ShardSpec::with_range(vec![0x40], vec![0x60]);
