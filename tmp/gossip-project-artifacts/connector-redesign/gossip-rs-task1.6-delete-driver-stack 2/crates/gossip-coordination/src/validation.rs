...
pub fn validate_cursor_update_pooled(
        ...
        || (!spec_end.is_empty() && new_last_key >= spec_end)
    {
        return Err(CoordError::CursorOutOfBounds(
            crate::error::CursorOutOfBoundsDetail {
...
mod tests {
    ...
    use super::*;
    use crate::lease::{LeaseHolder, OpKind, OpResult};
    use crate::record::ShardRecord;
    use crate::test_fixtures::{other_tenant, test_run, test_shard, test_spec, test_tenant};
    use gossip_contracts::coordination::cursor::{
    ...
    use gossip_contracts::identity::{FenceEpoch, LogicalTime, OpId, WorkerId};
    ...
    fn active_leased_record(slab: &mut ByteSlab) -> ShardRecord {
        let r = ShardRecord::from_raw_parts(
            ...
            test_run(),
            test_shard(),
            crate::record::ShardStatus::Active,
            None,
            &test_spec(),
    ...
    fn validate_lease_terminal_shard() {
        ...
        record.status = crate::record::ShardStatus::Done;
    ...
    proptest! {
        ...
        fn prop_validate_cursor_update_matches_spec(
            ...
            spec in gossip_contracts::test_util::arb_bounded_shard_spec(),
        ) {
