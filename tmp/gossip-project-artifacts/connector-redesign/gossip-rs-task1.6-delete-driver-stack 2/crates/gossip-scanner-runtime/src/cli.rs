...
use crate::{
    ...
    GitDebugLevel, ScanReport, ScanRuntimeError, TransformFilter, available_workers, scan_fs, scan_git,
};
...
pub fn run(config: CliConfig) -> Result<ScanReport, CliError> {
    ...
    let report = match config.source {
        ...
        CliSource::Git { ref repo } => scan_git(
