...
//! routes scans into the family-oriented runtime boundaries.
...
/// entrypoints exercise the new family-oriented runtime surface.
...
mod tests {
    ...
    #[test]
    fn run_worker_returns_placeholder_error_for_filesystem_family() {
        ...
        let error = run_worker(&cfg).expect_err("ordered-content runtime is placeholder");
        assert!(error.to_string().contains("ordered-content"));
    }

    #[test]
    fn run_worker_returns_placeholder_error_for_git_family() {
        let dir = tempdir().expect("tempdir");
        ...
        let error = run_worker(&cfg).expect_err("git runtime is placeholder");
