...
pub enum SplitValidationError {
    ...
    ///
    /// This is distinct from [`MAX_SPLIT_CHILDREN`]: the global contract limit
    /// bounds plan shape, while a concrete backend may choose a stricter limit
    /// for one atomic publication operation.
...
impl fmt::Display for SplitValidationError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::BackendChildLimitExceeded { count, max } => write!(
                f,
                "backend child fanout exceeded: requested {count} children, max {max}",
            ),
