...
use gossip_contracts::coordination::{CursorSemantics, CursorUpdate, PooledSpawned, ShardSpec, ShardSpecRef, SplitReplaceChild, SplitReplacePlan, SplitResidualPlan};
...
use gossip_coordination::{
    ...
    OpKind, OpLogEntry, OpResult, ParkReason, RunConfig, RunManagement, RunOpKind,
...
fn config_rejects_empty_endpoint_list() {
    let err = EtcdCoordinatorConfig::new(Vec::<String>::new(), "/gossip/v1", 60, 8, 8)
...
fn config_rejects_invalid_namespace_prefix() {
    let err = EtcdCoordinatorConfig::new(["http://127.0.0.1:2379"], "gossip/v1", 60, 8, 8)
...
fn split_replace_round_trip_against_local_etcd() {
    ...
    let replay = backend
        .split_replace(now(5), test_tenant(), &acquire.lease, plan, op)
...
fn split_residual_round_trip_against_local_etcd() {
    ...
    assert_eq!(parent.spawned.iter(&parent_slab).next(), Some(first.as_ref().residual));
    ...
    let replay = backend
        .split_residual(now(6), test_tenant(), &acquire.lease, plan, op)
...
fn active_run_index_hides_initializing_runs_and_unpublishes_terminal_runs() {
    ...
    let config = RunConfig::try_new(CursorSemantics::Completed, 30, Some(5)).unwrap();
    let run_init = RunId::from_raw(0x2000);
    let run_active = RunId::from_raw(0x2001);
    let run_cancelled = RunId::from_raw(0x2002);

    backend
        .create_run(now(1), test_tenant(), run_init, config)
        .expect("create_run should succeed");
    let mut active = Vec::new();
    backend
        .list_active_runs_into(test_tenant(), &mut active)
        .expect("active run listing should succeed");
    assert!(active.is_empty(), "initializing runs must not be published in active index");

    backend
        .create_run(now(2), test_tenant(), run_active, config)
        .expect("create_run should succeed");
    let manifest_active = [InitialShardInput::new(
        ShardId::from_raw(0x8100),
        ShardSpecRef::new(b"a", b"m", b""),
        CursorUpdate::initial(),
    )];
    backend
        .register_shards(now(3), test_tenant(), run_active, &manifest_active, OpId::from_raw(1))
        .expect("register_shards should succeed");
    ...
    backend
        .create_run(now(4), test_tenant(), run_cancelled, config)
        .expect("create_run should succeed");
    let cancel = backend
        .cancel_run(now(5), test_tenant(), run_cancelled, OpId::from_raw(2))
        .expect("cancel_run should succeed");
    assert!(cancel.is_executed());

    active.clear();
    backend
        .list_active_runs_into(test_tenant(), &mut active)
        .expect("active run listing should succeed");
    assert_eq!(active, vec![run_active], "cancelled initializing runs must never appear in active index");

    let complete = backend
        .complete_run(now(6), test_tenant(), run_active, OpId::from_raw(3))
    ...
    backend
        .list_active_runs_into(test_tenant(), &mut active)
        .expect("active run listing should succeed");
    assert!(active.is_empty(), "terminal runs must be removed from active index");
}
...
fn fail_run_removes_active_index_and_replays_idempotently() {
    ...
    let active = backend
        .list_active_runs(test_tenant())
        .expect("active run listing should succeed");
    assert!(active.is_empty(), "failed run must be removed from active index");
}
...
fn gc_stale_initializing_runs_deletes_only_old_orphans() {
    ...
    backend
        ...
        .expect("create_run should succeed");
    backend
        .create_run(now(20), test_tenant(), run_fresh, config)
        .expect("create_run should succeed");
    backend
        .create_run(now(30), test_tenant(), run_active, config)
        .expect("create_run should succeed");
    let manifest = [InitialShardInput::new(
    ...
    backend
        .register_shards(now(31), test_tenant(), run_active, &manifest, OpId::from_raw(21))
        .expect("register_shards should succeed");

    let deleted = backend
        .gc_stale_initializing_runs(test_tenant(), now(10), 16)
        .expect("gc should succeed");
    assert_eq!(deleted, vec![run_old]);

    let old_lookup = backend.get_run(test_tenant(), run_old);
    assert!(matches!(old_lookup, Err(gossip_coordination::GetRunError::RunNotFound)));

    let fresh = backend
        .get_run(test_tenant(), run_fresh)
        .expect("fresh initializing run must remain");
    assert_eq!(fresh.status(), RunStatus::Initializing);

    let active = backend
        .get_run(test_tenant(), run_active)
        .expect("active run must remain");
    assert_eq!(active.status(), RunStatus::Active);

    let listed = backend
        .list_active_runs(test_tenant())
        .expect("active run listing should succeed");
    assert_eq!(listed, vec![run_active]);
}
...
fn unpark_seeded_parked_shard_round_trip_against_local_etcd() {
    ...
    assert!(active.fence_epoch > fence_before, "unpark must bump fence epoch");
