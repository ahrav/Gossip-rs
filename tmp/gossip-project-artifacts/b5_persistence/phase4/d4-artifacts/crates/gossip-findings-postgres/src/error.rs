...
pub enum FindingsPgError {
    ...
    /// A supposedly fixed-width identity column had an unexpected byte length.
    InvalidByteLength {
        column: &'static str,
        expected: usize,
        actual: usize,
    },
    /// A caller supplied a query limit that does not fit into PostgreSQL BIGINT.
    LimitOutOfRange { limit: usize },
    /// A PostgreSQL BIGINT -> u64 conversion failed while decoding a read row.
    PgIntConversion(PgIntConversionError),
...
impl fmt::Display for FindingsPgError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::InvalidByteLength { column, expected, actual } => write!(
                f,
                "column {column} had invalid byte length {actual} (expected {expected})"
            ),
            Self::LimitOutOfRange { limit } => {
                write!(f, "query limit {limit} does not fit into PostgreSQL BIGINT")
            }
            Self::PgIntConversion(err) => write!(f, "PostgreSQL integer decode failed: {err}"),
...
impl From<PgIntConversionError> for FindingsPgError {
    fn from(value: PgIntConversionError) -> Self {
        Self::PgIntConversion(value)
    }
}
