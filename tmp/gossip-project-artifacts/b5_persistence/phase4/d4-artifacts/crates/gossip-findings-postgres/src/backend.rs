...
use crate::{
    ...
    pg_int::pg_i64_nonnegative_to_u64,
    read_api::{ObservationCountByPolicy, PendingTriageFinding},
    schema::{
        COUNT_OBSERVATIONS_BY_TENANT_POLICY_SQL, FINDINGS_INSERT_SQL, FINDINGS_TABLE, FindingRow,
        LIST_FINDINGS_NEEDING_TRIAGE_SQL, OBSERVATIONS_COUNT_SQL,
...
impl FindingsSinkPg {
    ...
    /// Count durable observations for a tenant, grouped by policy hash.
    ///
    /// This is the minimal D4 read surface used by correctness checks and
    /// workload harnesses.
    pub fn count_observations_by_tenant_policy(
        ...
        for row in rows {
            let tenant = decode_id32(row.get::<_, Vec<u8>>(0), "tenant_id")?;
            let policy = decode_id32(row.get::<_, Vec<u8>>(1), "policy_hash")?;
            let count_i64: i64 = row.get(2);
            let count = pg_i64_nonnegative_to_u64(count_i64, "observation_count")?;
            out.push(ObservationCountByPolicy::new(
                TenantId::from_bytes(tenant),
                PolicyHash::from_bytes(policy),
                count,
            ));
        }
        Ok(out)
    ...
    /// Placeholder query for "findings needing triage" until mutable triage
    /// state exists.
    ///
    /// Today this returns one latest observation per finding for the tenant,
    /// sorted by `(seen_at DESC, observation_id DESC)`.
    pub fn list_findings_needing_triage(
        &self,
        tenant_id: TenantId,
        limit: usize,
    ) -> Result<Vec<PendingTriageFinding>, FindingsPgError> {
        let limit_i64 = i64::try_from(limit).map_err(|_| FindingsPgError::LimitOutOfRange {
            limit,
        })?;
        let tenant_bytes: &[u8] = tenant_id.as_bytes();
        let mut client = self.lock_client()?;
        let rows = client.query(LIST_FINDINGS_NEEDING_TRIAGE_SQL, &[&tenant_bytes, &limit_i64])?;

        let mut out = Vec::with_capacity(rows.len());
        for row in rows {
            let tenant = TenantId::from_bytes(decode_id32(row.get::<_, Vec<u8>>(0), "tenant_id")?);
            let finding_id = FindingId::from_bytes(decode_id32(row.get::<_, Vec<u8>>(1), "finding_id")?);
            let stable_item_id = StableItemId::from_bytes(
                decode_id32(row.get::<_, Vec<u8>>(2), "stable_item_id")?,
            );
            let occurrence_id = OccurrenceId::from_bytes(decode_id32(row.get::<_, Vec<u8>>(3), "occurrence_id")?);
            let observation_id = ObservationId::from_bytes(decode_id32(row.get::<_, Vec<u8>>(4), "observation_id")?);
            let policy_hash = PolicyHash::from_bytes(decode_id32(row.get::<_, Vec<u8>>(5), "policy_hash")?);
            let seen_at = pg_i64_nonnegative_to_u64(row.get::<_, i64>(6), "seen_at")?;
            let location_display: Option<String> = row.get(7);
            let location_url: Option<String> = row.get(8);

            out.push(PendingTriageFinding::new(
                tenant,
                finding_id,
                stable_item_id,
                occurrence_id,
...
fn decode_id32(bytes: Vec<u8>, column: &'static str) -> Result<[u8; 32], FindingsPgError> {
    if bytes.len() != 32 {
