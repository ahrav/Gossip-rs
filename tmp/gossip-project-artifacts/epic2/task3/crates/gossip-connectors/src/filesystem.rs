...
//! - Resume is key-authoritative: `Cursor.last_key` is the only durable
//!   progress primitive. Incoming tokens are ignored and the connector always
//!   restarts strictly after `last_key` within the effective key range.
...
impl FilesystemConnector {
    ...
    pub fn fill_page(
        ...
        let shard_start = borrowed_shard_bound(shard.key_range_start(), "start")?;
        let shard_end = borrowed_shard_bound(shard.key_range_end(), "end")?;
        ...
        let bounds = common::resolve_bounds(&self.entries, effective_start, effective_end)?;
        ...
        let page = PageBuf::try_new_validated(
            items,
            state,
            effective_start.unwrap_or(&[]),
            effective_end.unwrap_or(&[]),
        )
        .map_err(|error| {
