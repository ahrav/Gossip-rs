...
use gossip_contracts::{
    connector::{
        GIT_CONNECTOR_TAG, ItemKey, derive_filesystem_stable_item_id, derive_stable_item_id,
    },
    coordination::ShardSpec,
    identity::{
        ConnectorInstanceIdHash, LogicalTime, OpId, PolicyHash, RunId, ShardId, TenantId,
        TenantSecretKey, WorkerId,
    },
};
...
fn channel_store_producer_sends_run_loss_and_end_run() {
    ...
    let (tx, rx) = sync_channel(16);
    let dir = tempdir().expect("tempdir");
    let producer = ChannelStoreProducer::try_new(tx, dir.path().to_path_buf())
        .expect("channel store producer should canonicalize tempdir root");
...
#[test]
fn filesystem_identity_scope_separates_same_relative_path_across_roots() {
    let left = tempdir().expect("left tempdir");
    let right = tempdir().expect("right tempdir");
    let item_key = ItemKey::try_from_slice(b"shared/secret.txt").expect("item key");
    let left_root = fs::canonicalize(left.path()).expect("canonicalize left root");
    let right_root = fs::canonicalize(right.path()).expect("canonicalize right root");

    let left_scope = FilesystemIdentityScope::from_canonical_root(&left_root);
    let right_scope = FilesystemIdentityScope::from_canonical_root(&right_root);

    assert_ne!(
        left_scope.stable_item_id(&item_key),
        right_scope.stable_item_id(&item_key),
        "different filesystem roots must produce distinct StableItemId values even for the same relative path",
    );
}

#[test]
fn filesystem_identity_scope_is_stable_within_root() {
    let root = tempdir().expect("tempdir");
    let canonical_root = fs::canonicalize(root.path()).expect("canonicalize root");
    let item_key = ItemKey::try_from_slice(b"shared/secret.txt").expect("item key");

    let scope = FilesystemIdentityScope::from_canonical_root(&canonical_root);

    assert_eq!(
        scope.stable_item_id(&item_key),
        scope.stable_item_id(&item_key),
        "stable-item derivation must be pure for the same root and relative path",
    );
}

#[test]
fn channel_store_producer_scopes_stable_ids_by_canonical_root() {
    use scanner_scheduler::store::{FsFindingBatch, StoreProducer};
    use std::sync::mpsc::sync_channel;

    let left = tempdir().expect("left tempdir");
    let right = tempdir().expect("right tempdir");
    let relative = "shared/secret.txt";

    let left_root = fs::canonicalize(left.path()).expect("canonicalize left root");
    let right_root = fs::canonicalize(right.path()).expect("canonicalize right root");
    let left_object_path = left_root.join(relative);
    let right_object_path = right_root.join(relative);
    let left_object_path_bytes = left_object_path.as_os_str().as_encoded_bytes().to_vec();
    let right_object_path_bytes = right_object_path.as_os_str().as_encoded_bytes().to_vec();

    let (left_tx, left_rx) = sync_channel(1);
    let (right_tx, right_rx) = sync_channel(1);
    let left_producer = ChannelStoreProducer::try_new(left_tx, left_root)
        .expect("left producer should canonicalize root");
    let right_producer = ChannelStoreProducer::try_new(right_tx, right_root)
        .expect("right producer should canonicalize root");

    left_producer
        .emit_fs_batch(FsFindingBatch {
            object_path: &left_object_path_bytes,
            findings: &[],
        })
        .expect("left emit_fs_batch should succeed");
    right_producer
        .emit_fs_batch(FsFindingBatch {
            object_path: &right_object_path_bytes,
            findings: &[],
        })
        .expect("right emit_fs_batch should succeed");

    let CommitMessage::Batch(left_batch) = left_rx.recv().expect("left commit batch") else {
        panic!("expected left commit batch message");
    };
    let CommitMessage::Batch(right_batch) = right_rx.recv().expect("right commit batch") else {
        panic!("expected right commit batch message");
    };

    assert_eq!(left_batch.object_path.as_slice(), b"shared/secret.txt");
    assert_eq!(right_batch.object_path.as_slice(), b"shared/secret.txt");
    assert_ne!(
        left_batch.stable_item_id, right_batch.stable_item_id,
        "same relative filesystem path under different roots must not collide",
    );
}

#[test]
fn channel_store_producer_uses_canonical_filesystem_connector_tag() {
    use scanner_scheduler::store::{FsFindingBatch, StoreProducer};
    use std::sync::mpsc::sync_channel;
    ...
    let object_path = canonical_root.join("nested/secret.txt");
    let object_path_bytes = object_path.as_os_str().as_encoded_bytes().to_vec();

    let (tx, rx) = sync_channel(1);
    let producer = ChannelStoreProducer::try_new(tx, canonical_root.clone())
        .expect("channel store producer should accept canonical root");
    producer
        .emit_fs_batch(FsFindingBatch {
            object_path: &object_path_bytes,
            findings: &[],
        })
        .expect("emit_fs_batch should succeed");
    drop(producer);

    let CommitMessage::Batch(batch) = rx.recv().expect("one commit batch") else {
        panic!("expected commit batch message");
    };

    let expected_item_key = ItemKey::try_from_slice(b"nested/secret.txt").expect("item key");
    let connector_instance = ConnectorInstanceIdHash::from_instance_id_bytes(
        canonical_root.as_os_str().as_encoded_bytes(),
    );
    let expected = derive_filesystem_stable_item_id(connector_instance, &expected_item_key);
    let wrong_tag = derive_stable_item_id(GIT_CONNECTOR_TAG, connector_instance, &expected_item_key);

    assert_eq!(batch.object_path.as_slice(), b"nested/secret.txt");
    assert_eq!(
        batch.stable_item_id, expected,
        "filesystem runtime identity derivation must use FILESYSTEM_CONNECTOR_TAG",
    );
    assert_ne!(
        batch.stable_item_id, wrong_tag,
        "filesystem runtime identity derivation must not drift to a different connector tag",
    );
}

#[test]
fn channel_store_producer_canonicalizes_equivalent_roots() {
    use std::sync::mpsc::sync_channel;

    let real_root = tempdir().expect("real_root tempdir");
    let spelled_root = real_root.path().join(".").join("nested").join("..");
    let canonical_real = fs::canonicalize(real_root.path()).expect("canonical real root");
    let canonical_spelled = fs::canonicalize(&spelled_root).expect("canonical spelled root");

    assert_eq!(canonical_real, canonical_spelled);

    let (tx_real, _rx_real) = sync_channel(1);
    let producer_real = ChannelStoreProducer::try_new(tx_real, real_root.path().to_path_buf())
        .expect("producer_real");
    let (tx_spelled, _rx_spelled) = sync_channel(1);
    let producer_spelled = ChannelStoreProducer::try_new(tx_spelled, spelled_root)
        .expect("producer_spelled");

    assert_eq!(
        producer_real.root, producer_spelled.root,
        "equivalent roots must canonicalize to the same stored root"
    );
    assert_eq!(
        producer_real.identity, producer_spelled.identity,
        "equivalent roots must hash to the same filesystem identity scope"
    );
}

#[test]
fn channel_store_producer_respects_explicit_connector_instance_override() {
    use scanner_scheduler::store::{FsFindingBatch, StoreProducer};
    use std::sync::mpsc::sync_channel;
    ...
    let connector_instance =
        ConnectorInstanceIdHash::from_instance_id_bytes(b"filesystem-connector-instance");
    let identity = FilesystemIdentityScope::from_connector_instance(connector_instance);
    ...
    let (left_tx, left_rx) = sync_channel(1);
    let left_producer = ChannelStoreProducer::try_new_with_identity(left_tx, left_root, identity)
        .expect("left producer");
    let (right_tx, right_rx) = sync_channel(1);
    let right_producer =
        ChannelStoreProducer::try_new_with_identity(right_tx, right_root, identity)
    ...
    assert_eq!(
        left_batch.stable_item_id, right_batch.stable_item_id,
        "explicit connector instance should keep stable ids aligned across roots"
    );
}
