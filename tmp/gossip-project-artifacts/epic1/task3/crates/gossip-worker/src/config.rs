...
//! - real distributed scans (`ExecutionMode::Connector` + `--backend=production`)
//!
//! Connector mode never silently falls back to a local or fake backend. The
//! caller must select a backend explicitly, and filesystem connector mode is
//! the only distributed source supported in MVP-A.
...
///
/// `Production` is the only valid backend for connector mode in MVP-A.
/// `Local` remains accepted so direct-mode invocations can state their intent
/// explicitly and so validation can fail with a precise message instead of an
...
impl LocalWorkerConfig {
    ...
    #[must_use]
    pub fn new(execution_mode: ExecutionMode, source: WorkerSourceSettings, budgets: ScanBudgets) -> Self {
        Self {
            execution_mode,
            source,
            budgets,
...
///
/// This configuration contains all explicit identity, source, and backend
/// settings required to run one worker against etcd + PostgreSQL without any
/// code changes or hidden production defaults.
#[derive(Clone, Debug)]
    ...
    fn resolve(self) -> Result<ResolvedWorkerConfig, WorkerConfigError> {
        ...
        match execution_mode {
            ExecutionMode::Direct => {
                if matches!(self.backend, Some(BackendSelection::Production)) {
                    return Err(WorkerConfigError::UnsupportedCombination {
                        message:
            ...
            ExecutionMode::Connector => {
                ...
                let backend = self.backend.ok_or(WorkerConfigError::MissingRequiredValue {
                    field: "backend",
                    flag: "--backend",
                ...
                if backend != BackendSelection::Production {
                    return Err(WorkerConfigError::UnsupportedCombination {
                        message:
                            "connector mode runs the distributed worker loop; use --backend=production for distributed filesystem scans or switch to --mode=direct for local scans"
                                .to_owned(),
                    });
                }
...
fn usage() -> &'static str {
    "usage: gossip-worker [--mode=direct|connector] [--backend=local|production] \\
     ...
     note: connector mode requires --backend=production and currently supports only fs; use --mode=direct for local fs/git scans"
}
...
#[derive(Default)]
struct NoopCoordinationEventRecorder;
...
impl CoordinationEventRecorder for NoopCoordinationEventRecorder {
    ...
    fn record_commit_progress(&self, _shard_id: &str, _event: CommitProgressRecord) -> AnyhowResult<()> {
        Ok(())
    }
}
...
mod tests {
    ...
    fn connector_mode_requires_explicit_backend_selection() {
        ...
        assert!(
            ...
            "error should explain that backend selection is required: {err}"
        );
    }
