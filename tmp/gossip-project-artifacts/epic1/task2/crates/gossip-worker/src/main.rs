...
//!
//! The worker now resolves a typed configuration surface from environment
//! variables plus CLI overrides. Depending on that resolved config it either:
//!
//! - runs a local filesystem/git scan, or
//! - launches the real distributed worker path against etcd + PostgreSQL.

use std::fmt;

use gossip_scanner_runtime::{ScanRuntimeError, scan_fs, scan_git};
use gossip_worker::config::{
    LocalWorkerConfig, ResolvedWorkerConfig, WorkerSourceSettings,
...
fn run_local_worker(cfg: &LocalWorkerConfig) -> Result<(u64, u64, u64), WorkerError> {
    let report = match cfg.source() {
        WorkerSourceSettings::Fs(source) => scan_fs(
            &source.to_scan_config(cfg.execution_mode(), cfg.budgets()),
        )?,
        WorkerSourceSettings::Git(source) => scan_git(
            &source.to_scan_config(cfg.execution_mode(), cfg.budgets()),
        )?,
    };
    Ok((
...
fn log_distributed_report(
    cfg: &gossip_worker::config::DistributedWorkerConfig,
...
mod tests {
    ...
    fn local_worker_scans_filesystem_path() {
        ...
        let cfg = LocalWorkerConfig::new(
            ...
            WorkerSourceSettings::Fs(
                gossip_worker::config::FsSourceSettings::new(dir.path().to_path_buf()),
            ),
    ...
    fn local_worker_scans_git_repo_path() {
        ...
        let cfg = LocalWorkerConfig::new(
            ExecutionMode::Connector,
            WorkerSourceSettings::Git(
                gossip_worker::config::GitSourceSettings::new(dir.path().to_path_buf()),
            ),
            gossip_scanner_runtime::ScanBudgets::default(),
        );
