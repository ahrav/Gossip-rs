...
///
/// - `Local` preserves the existing non-production connector path and is valid
///   only when selected explicitly.
/// - `Production` requires the real etcd and PostgreSQL backends.
    ...
    fn resolve(self) -> Result<ResolvedWorkerConfig, WorkerConfigError> {
        ...
        match execution_mode {
            ...
            ExecutionMode::Connector => {
                ...
                match backend {
                    BackendSelection::Local => Ok(ResolvedWorkerConfig::Local(LocalWorkerConfig::new(
                        ExecutionMode::Connector,
                        build_local_source_settings(
                            source,
                            path,
                    ...
                    BackendSelection::Production => {
                        if source != WorkerSource::Fs {
                            return Err(WorkerConfigError::UnsupportedCombination {
                                message:
                                    "backend=production currently supports only source=fs in MVP-A"
                                        .to_owned(),
                            });
                        }
...
mod tests {
    ...
    fn production_rejects_git_source() {
        ...
        let err = resolve_worker_config_from(Vec::<String>::new(), &env)
            .expect_err("production backend must reject git until Epic 3");

        assert!(
            err.to_string().contains("source=fs"),
            "error should explain the current MVP-A source restriction: {err}"
        );
    }
