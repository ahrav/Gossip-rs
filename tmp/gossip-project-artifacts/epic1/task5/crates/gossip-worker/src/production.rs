...
pub fn build_production_backends(
    ...
    build_production_backends_from_clients(config.etcd().clone(), done_ledger_client, findings_client)
}
...
mod tests {
    ...
    fn run_production_worker_uses_real_backends_and_never_falls_back() {
        ...
        fs::write(dir.path().join("secret.txt"), "token=step5-real-backend-proof")
