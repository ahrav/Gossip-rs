...
use gossip_scan_driver::{
    Assignment, AssignmentSource, CancellationToken, CommitSink, ConnectorKind,
...
pub mod cli;
/// Shared family-neutral commit vocabulary for the runtime durability path.
pub mod commit_model;
/// Holds non-durable commit-sink compatibility shims for local/CLI scan paths.
pub mod commit_sink;
/// Receipt-driven committed-prefix checkpoint aggregation.
pub mod checkpoint_aggregator;
/// Bounded execution → commit pipeline for receipt-driven durability.
pub mod commit_pipeline;
/// Deterministic translation from scan results into persistence-layer rows.
pub mod result_translation;
/// Implements the authoritative findings → done-ledger durable commit stage.
...
/// worker (for coordinator completion). Any driver-local checkpoint hints are
/// intentionally *not* surfaced here: Epic 2 locks runtime progress to durable
/// receipts rather than scan-completion side channels.
#[derive(Clone, Debug)]
...
#[cfg(test)]
#[path = "runtime_durability_tests.rs"]
