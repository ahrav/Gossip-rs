...
impl CommitScope {
    ...
    /// Construct the commit scope from a shared write context.
    ///
    /// Runtime code should prefer this over splatting raw lease metadata into
    /// loose arguments. The write context carries routing and fencing scope;
    /// the commit scope adds only the committed-prefix length plus the
    /// family-neutral checkpoint boundary.
    #[must_use]
...
mod tests {
    ...
    #[test]
    fn commit_scope_from_write_context_uses_fence_fields() {
        ...
        assert_eq!(scope.tenant_id(), write_context.tenant_id());
        assert_eq!(scope.run_id(), write_context.run_id());
        assert_eq!(scope.shard_id(), write_context.shard_id());
        assert_eq!(scope.fence_epoch(), write_context.fence_epoch());
        assert_eq!(scope.committed_units(), 5);
        assert_eq!(scope.checkpoint_cursor(), &frontier);
    }
