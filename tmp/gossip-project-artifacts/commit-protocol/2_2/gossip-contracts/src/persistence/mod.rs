...
//! - `page_commit.rs` defines the family-neutral checkpoint boundary types and the
//!   `PageCommit<S>` typestate machine that enforces findings → done-ledger →
//!   checkpoint ordering.
//! - `write_context.rs` defines the shared write-side fencing metadata carried
//!   through runtime and persistence APIs.
...
//! frontier boundary after both layers are durable. The `PageCommit<S>` typestate machine enforces that ordering.
//!
...
pub use page_commit::{
    ...
    PageCommitScope, PageCommitValidationError,
};
