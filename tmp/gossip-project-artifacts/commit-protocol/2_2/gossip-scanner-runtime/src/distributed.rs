...
use gossip_contracts::{
    identity::{FenceEpoch, RunId, ShardId, TenantSecretKey},
...
/// secret key needed for downstream identity derivation. One lease corresponds
...
mod tests {
    ...
    fn run_worker_persists_identity_chain_and_marks_done() {
        ...
        assert!(
            identities
                .iter()
                .all(|(_, record)| record.write_context == done_mark_contexts[0]),
            "all identity writes should carry the same shared write context as done-ledger completion"
        );
    }
