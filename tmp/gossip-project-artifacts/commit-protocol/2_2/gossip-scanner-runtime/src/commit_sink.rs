...
impl DurableCommitSink {
    ...
    ///
    /// All identity derivation uses the tenant/routing scope from
    /// `write_context` plus `tenant_secret_key`, while stable item identity is
    /// trusted from connector-provided [`ItemMeta`]. Derived records are
    /// persisted through `recorder`.
    #[must_use]
    pub fn new(
        recorder: std::sync::Arc<dyn CoordinationEventRecorder>,
        shard_id: Arc<str>,
    ...
    fn durable_commit_sink_derives_identity_chain() {
        ...
        let sink = DurableCommitSink::new(
            ...
            WriteContext::new(
                ...
                gossip_contracts::identity::PolicyHash::from_bytes([0x12; 32]),
                gossip_contracts::identity::RunId::from_raw(13),
                gossip_contracts::identity::ShardId::from_raw(14),
                gossip_contracts::identity::FenceEpoch::from_raw(15),
            ),
        ...
        match &progress[0] {
            CommitProgressRecord::Begin { write_context, .. } => {
                assert_eq!(
                    *write_context,
                    WriteContext::new(
                        TenantId::from_bytes([0x11; 32]),
