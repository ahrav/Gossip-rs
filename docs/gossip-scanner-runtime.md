...
| `src/coordination_sink.rs` | Owned event records (`StoredGitEvent`, `CommitProgressRecord`), `CoordinationEventRecorder`, and `FindingsCaptureSink`/`GitFindingForPersistence` adapters for repo-frontier Git finding capture before durable translation |
