...
|------|--------|------------------|
| `src/timing_wheel_tests.rs` | 14 | Never-fires-early, pool coherence, horizon enforcement, slot occupancy, FIFO ordering, monotonicity, reset correctness, Bitset2 consistency |
| `src/bitset_tests.rs` | 12 | Set/unset roundtrips, padding invariant preservation, count correctness, clear semantics, iterator consistency |
| `src/inline_vec.rs` | 9 | Push/pop/truncate/extend/drain bounds, `get_unchecked` equivalence, `MaybeUninit` array validity |
| `src/spsc.rs` | 8 | Ring capacity, index wrapping, push/pop safety, producer/consumer non-interference, empty/full detection |
| `src/atomic_bitset_tests.rs` | 6 | `test_and_set` idempotency, bit independence, count bounds, clear correctness |
| `src/atomic_seen_sets_tests.rs` | 5 | `mark`/`is_seen` roundtrips, cross-set independence, clear-all |

### scanner-engine (15 proofs)

| File | Proofs | What is verified |
|------|--------|------------------|
| `src/scratch_memory.rs` | 6 | `ScratchVec` push/truncate/pop/extend/drain bounds, `get_unchecked` safety |
| `src/pool/node_pool.rs` | 5 | In-bounds acquire, non-overlapping nodes, release/reacquire, free-count tracking, alignment |
| `src/engine/hit_pool.rs` | 4 | `push_span` OOB safety, coalesce path correctness, `take_into` bounds, `reset_touched` bitset bounds |

### scanner-scheduler (9 proofs)

| File | Proofs | What is verified |
|------|--------|------------------|
| `src/archive/detect.rs` | 5 | `detect_kind_from_name_bytes` panic-freedom, suffix detection functions (`_z`, `_r`, `_p`, `_2`) panic-freedom |
| `src/runtime.rs` | 2 | Chunk view arithmetic bounds, file chunk copy window fitting |
| `src/archive/budget.rs` | 1 | Budget stack `depth - 1` index validity for any enter/exit sequence |
| `src/archive/formats/tar.rs` | 1 | Zero-block detection `ptr.add(i + j)` stays within `[0, 512)` |

### scanner-git (2 proofs)

| File | Proofs | What is verified |
|------|--------|------------------|
| `src/runner_exec.rs` | 1 | `shard_id_for_exec_position` produces valid, complete, balanced partitioning |
| `src/pack_plan.rs` | 1 | DAG `remaining_out` / `dag_remaining` counters never underflow |
