...
**File**: `crates/scanner-engine/src/engine/window_validate.rs`
...
- Suppression requires both an `Invalid` verdict
...
Located in `crates/scanner-engine/src/engine/window_validate_tests.rs` (included via `#[path]` attribute).
