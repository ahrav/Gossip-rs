...
| **Archive Core**               | `crates/scanner-scheduler/src/archive/` (`mod.rs`, `scan.rs`, `budget.rs`, `config.rs`, `detect.rs`, `outcome.rs`, `path.rs`, `util.rs`, `formats/*`) | Archive scanning config, budgets, outcomes, path canonicalization, and sink-driven scan core                    |
