...
| Additional synthetic coverage | `crates/scanner-engine-integration-tests/tests/simulation/` scanner-focused `sim-harness` modules (see `tests/simulation/main.rs` and `docs/scanner-engine-integration-tests.md`) | Archive, discovery fallback, size or budget invariants, and mutation-pipeline coverage           |
