...
|------|----------|-------------|
| `TreeDiffWalker` | `tree_diff.rs:269` | Stateful walker that diffs two trees using an explicit stack. Reusable across calls. |
| `TreeDiffStats` | `tree_diff.rs:87` | Cumulative counters: trees loaded, bytes loaded, peak in-flight, candidates emitted, subtrees skipped, max depth. |
| `TreeDiffLimits` | `tree_diff_limits.rs:31` | Hard caps for depth, in-flight bytes, candidate count, path arena, tree cache, delta cache, and spill arena. |
| `TreeDiffError` | `errors.rs:195` | Error enum covering corrupt trees, depth/budget/path limits, candidate buffer full, object store failures, and abort. |

### Tree Entries

| Type | Location | Description |
|------|----------|-------------|
| `EntryKind` | `tree_entry.rs:70` | Enum classifying tree entry type: `Tree`, `RegularFile`, `ExecutableFile`, `Symlink`, `Gitlink`, `Unknown`. |
| `TreeEntry<'a>` | `tree_entry.rs:119` | Zero-copy parsed entry: name, OID bytes, kind, mode. Borrows from tree buffer. |
| `TreeEntryIter<'a>` | `tree_entry.rs:237` | Fused iterator over entries in a raw tree payload. No per-entry allocation. |
| `ParsedTreeEntry` | `tree_entry.rs:162` | Internal offset-based parsed entry. Used by `BufferedCursor` and `TreeStream` for peek/advance. |
| `ParseOutcome` | `tree_entry.rs:222` | `Complete(ParsedTreeEntry)` or `Incomplete(ParseStage)` for streaming callers that may refill buffers. |
| `ParseStage` | `tree_entry.rs:200` | Which parsing stage ran out of bytes: `Mode`, `Name`, or `Oid`. |

### Ordering

| Type | Location | Description |
|------|----------|-------------|
| `git_tree_name_cmp` | `tree_order.rs:97` | Compares two entry names using Git's `base_name_compare()` semantics with virtual terminators. |
| `git_tree_file_name_cmp` | `tree_order.rs:152` | Optimized file-only comparison (both `is_dir=false`); equivalent to lexicographic byte ordering. |

### Streaming

| Type | Location | Description |
|------|----------|-------------|
| `TreeStream<R: Read>` | `tree_stream.rs:71` | Single-pass streaming parser with a fixed-size sliding window buffer. Parses one entry ahead. |
| `TreeBytesReader` | `tree_stream.rs:30` | Adapts `TreeBytes` (cached, owned, or spilled) to `std::io::Read` without copying. |

### Candidates

| Type | Location | Description |
|------|----------|-------------|
| `ChangeKind` | `tree_candidate.rs:19` | `Add` (new blob at path) or `Modify` (existing blob changed). |
| `CandidateSink` | `tree_candidate.rs:114` | Trait for receiving emitted candidates. `emit()` receives OID, path, commit context, and flags. |
| `CandidateBuffer` | `tree_candidate.rs:139` | In-memory sink backed by a `Vec<TreeCandidate>` and a `ByteArena` for interned paths. |
| `TreeCandidate` | `tree_candidate.rs:80` | OID + `CandidateContext` (commit id, parent idx, change kind, mode flags, path ref). |
| `CandidateContext` | `tree_candidate.rs:51` | Stable context carried through spill/merge: commit, parent, change kind, mode, classification bits, arena path ref. |
| `ResolvedCandidate<'a>` | `tree_candidate.rs:93` | Dereferenced form with path bytes resolved from the arena. Produced by `iter_resolved()`. |

### Caching

| Type | Location | Description |
|------|----------|-------------|
| `TreeCache` | `tree_cache.rs:70` | Set-associative cache for decompressed tree payloads, keyed by OID. CLOCK eviction, fixed slots. |
| `TreeCacheHandle` | `tree_cache.rs:87` | Pinned handle to a cached tree payload. Dropping releases the pin. |
| `TreeDeltaCache` | `tree_delta_cache.rs:99` | Set-associative cache for delta base bytes, keyed by `(pack_id, offset)`. CLOCK eviction. |
| `TreeDeltaCacheHandle` | `tree_delta_cache.rs:119` | Pinned handle carrying base bytes, resolved `ObjectKind`, and `chain_len`. |

### Object Store Integration

| Type | Location | Description |
|------|----------|-------------|
| `TreeSource` | `object_store.rs:217` | Trait for loading raw tree payloads by OID. Returns `TreeBytes`. |
| `TreeBytes` | `object_store.rs:238` | Enum: `Cached(TreeCacheHandle)`, `Owned(Vec<u8>)`, or `Spilled(SpillSlice)`. |

## Diffing Algorithm

### Merge-Walk

`TreeDiffWalker::diff_trees` (`tree_diff.rs:361`) performs an iterative
...
The `compute_action` function (`tree_diff.rs:759`) computes the `Action` enum
...
kind matrix (`handle_matched_entries`, `tree_diff.rs:594`):
...
(`tree_order.rs:97`) implements Git's `base_name_compare()` semantics:
...
bytes are greater. This means `git_tree_file_name_cmp` (`tree_order.rs:152`)
...
type bits (`classify_mode`, `tree_entry.rs:473`):
...
The `parse_entry` function (`tree_entry.rs:358`) uses SIMD-accelerated
...
`TreeEntryIter` (`tree_entry.rs:237`) wraps `parse_entry` for complete
...
The walker uses a `TreeCursor` enum (`tree_diff.rs:214`) that selects between
...
`TreeStream` (`tree_stream.rs:71`) maintains a sliding window `buf[start..end]`
over a `Read` source:
...
16 KiB, `tree_diff.rs:79`) and never grows.
...
`TreeCache` (`tree_cache.rs:70`) stores decompressed tree payloads keyed by
...
`TreeDeltaCache` (`tree_delta_cache.rs:99`) stores decompressed delta base
...
multiply-shift rounds (`tree_delta_cache.rs:81`).

Default capacity: 128 MB (`TreeDiffLimits::DEFAULT.max_tree_delta_cache_bytes`).

### TreeBytes Variants

The `TreeSource` trait returns `TreeBytes` (`object_store.rs:238`), which may be:
...
`TreeDiffLimits` (`tree_diff_limits.rs:31`) enforces hard caps. All limits
...
Validation (`tree_diff_limits.rs:130`) enforces both minimum bounds (must allow
...
|----------|-------|----------|---------|
| `MAX_PATH_LEN` | 4096 | `tree_diff.rs:77` | Maximum path length in bytes (matches Linux `PATH_MAX`) |
| `TREE_STREAM_BUF_BYTES` | 16,384 | `tree_diff.rs:79` | Streaming parser buffer size |

### Error Recovery

On any error during `diff_trees`, the walker calls `cleanup_after_diff_call`
(`tree_diff.rs:541`) which drains all stack frames, releases in-flight byte
...
|---------|------------|------------|
| Diff walker, merge-walk algorithm | `tree_diff.rs` | 259-494 |
| Action dispatch and subtree push | `tree_diff.rs` | 117-144, 556-707 |
| `compute_action` (entry comparison) | `tree_diff.rs` | 759-810 |
| Tree entry binary format and parsing | `tree_entry.rs` | 1-64, 348-413 |
| `EntryKind` and mode classification | `tree_entry.rs` | 68-112, 462-489 |
| `TreeEntryIter` (complete-buffer iteration) | `tree_entry.rs` | 237-346 |
| Git tree ordering (`base_name_compare`) | `tree_order.rs` | 1-122 |
| `TreeStream` (sliding-window parser) | `tree_stream.rs` | 66-209 |
| `TreeBytesReader` (Read adapter) | `tree_stream.rs` | 30-64 |
| `TreeDiffLimits` and validation | `tree_diff_limits.rs` | 24-250 |
| `ChangeKind`, `CandidateSink`, `CandidateBuffer` | `tree_candidate.rs` | 17-260 |
| `TreeCandidate`, `CandidateContext`, `ResolvedCandidate` | `tree_candidate.rs` | 46-108 |
| `TreeCache` (OID-keyed set-associative cache) | `tree_cache.rs` | 62-152 |
| `TreeDeltaCache` (offset-keyed set-associative cache) | `tree_delta_cache.rs` | 91-215 |
| `SetAssociativeCache` framework (CLOCK eviction) | `cache_common.rs` | 1-61 |
| `TreeSource` trait, `TreeBytes` enum | `object_store.rs` | 211-294 |
| `TreeDiffError` variants | `errors.rs` | 195-226 |
