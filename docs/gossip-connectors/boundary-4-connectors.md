...
### EnumerationConnector (`api.rs:355-412`)
...
### ConnectorInstance (`api.rs:511-513`)
...
### ConnectorCapabilities (`api.rs:315-342`)
...
### ToxicDigest (`page_validator.rs:143-196`)
...
### PageValidationError (`page_validator.rs:374-490`)
...
Compile-time guard (`page_validator.rs:759`) asserts size <= 256 bytes.
...
`path_buf_from_bytes(bytes: &[u8]) -> PathBuf` (`common.rs:709-721`)
...
`common.rs` (1,024 lines) contains all shared connector infrastructure.
...
`scan_driver.rs` (1,223 lines) provides concrete `ScanSourceFactory`
